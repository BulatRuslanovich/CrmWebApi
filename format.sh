...
dotnet format CrmWebApi.csproj --verify-no-changes || {
    log_error "Code does not follow formatting rules!"
    log_step "Applying auto-formatting..."
    dotnet format CrmWebApi.csproj --verbosity minimal
}

log_step "Checking code style..."
dotnet format CrmWebApi.csproj --verify-no-changes --verbosity diagnostic 2>&1 | grep -E "(error|warning|violating)" || true
