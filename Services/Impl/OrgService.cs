...
public class OrgService(IOrgRepository repo, IMemoryCache cache, ILogger<OrgService> logger) : IOrgService
{
    ...
    public async Task<Result<PagedResponse<OrgResponse>>> GetAllAsync(int page, int pageSize)
    {
    ...
    public async Task<Result<IEnumerable<OrgTypeResponse>>> GetAllTypesAsync()
    {
        if (cache.TryGetValue(OrgTypesKey, out IEnumerable<OrgTypeResponse>? cached))
            return Result<IEnumerable<OrgTypeResponse>>.Success(cached!);
        ...
        return Result<IEnumerable<OrgTypeResponse>>.Success(result);
    }
